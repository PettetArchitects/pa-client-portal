...
export default function Shell({ projectName }) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {/* Top bar — glass */}
      <header className="h-14 border-b border-white/20 backdrop-blur-xl bg-white/30 flex items-center px-6 shrink-0" style={{ position: 'relative', zIndex: Z.CHROME }}>
      ...
      <div className="flex flex-1" style={{ position: 'relative', zIndex: Z.CHROME }}>
        {/* Sidebar — glass */}
        <nav className="w-52 border-r border-white/20 backdrop-blur-xl bg-white/30 py-6 px-3 shrink-0 hidden md:flex md:flex-col md:justify-between">
        ...
        {/* Mobile nav — glass */}
        <nav className="md:hidden fixed bottom-0 left-0 right-0 backdrop-blur-xl bg-white/30 border-t border-white/20 z-50 flex justify-around py-2 px-4">
          {navItems.map(({ to, icon: Icon, label }) => (
            <NavLink
              key={to}
              to={to}
